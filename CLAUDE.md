...
```
codex/automate 先更新main（git fetch origin + git pull --ff-only origin main），再 rebase main（git rebase main），然后读取 docs/tasks/Pxxx.md
...
```
mimo/work 先更新main（git fetch origin + git pull --ff-only origin main），再 rebase main（git rebase main），然后读取 docs/tasks/Pxxx.md
→ 严格按任务文档执行修改（无需中途停顿确认）
...
```
deveco/execute 先更新main（git fetch origin + git pull --ff-only origin main），再 rebase main（git rebase main），然后读取 docs/tasks/Pxxx.md
→ 严格按任务文档执行修改（无需中途停顿确认）
...
```
claude/review 先更新main（git fetch origin + git pull --ff-only origin main），再 rebase main（git rebase main），然后创建 docs/reviews/Pxxx-review.md
...
```
claude/review 同步 main 并 rebase
...
→ 对应工作分支（codex/automate / mimo/work / deveco/execute）同步 main 并 rebase
→ 根据 Review 修复
→ git add .
